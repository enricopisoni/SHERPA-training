...
def step1_potency(conf):
    ...
    prctileVec1=np.repeat(5,5)#np.array([35, 35, 35, 35, 35]);
    prctileVec2=np.repeat(20,5);
    prctileVec3=np.repeat(40,5);
    prctileVec4=np.repeat(60,5);
    prctileVec5=np.repeat(80,5);
    prctileVec6=np.repeat(95,5);
    ...
    for precursor in range(0, nPrec):
        ...
        prc2=np.percentile(potency[np.isfinite(potency)],prctileVec2[precursor]);
        prc3=np.percentile(potency[np.isfinite(potency)],prctileVec3[precursor]);
        prc4=np.percentile(potency[np.isfinite(potency)],prctileVec4[precursor]);
        prc5=np.percentile(potency[np.isfinite(potency)],prctileVec5[precursor]);
        prc6=np.percentile(potency[np.isfinite(potency)],prctileVec6[precursor]);                          
        #prc9=np.percentile(potency[np.isfinite(potency)],prctileVec2[precursor]);        
        
        speed=potency.copy();
        speed[np.isnan(speed)]=0
        potency[speed<prc1]=1;
        potency[(speed>=prc1) & (speed<prc2)]=2;
        potency[(speed>=prc2) & (speed<prc3)]=3;
        potency[(speed>=prc3) & (speed<prc4)]=4;
        potency[(speed>=prc4) & (speed<prc5)]=5;
        potency[(speed>=prc5) & (speed<prc6)]=6;                
        potency[speed>=prc6]=7;        
        val=categories;
