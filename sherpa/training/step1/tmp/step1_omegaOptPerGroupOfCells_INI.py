...
def step1_omegaOptPerGroupOfCells_iniRes(conf):
    ...
    step = conf.stepOptPerGroupCells_INI
...
#        flagIC = 0
        ...
        #                    print(inp1.shape)
                ...
                if inp2.size==0:
                    omega[ir:ir+step, ic:ic+step, PREC] = conf.step1_omegaOptPerPoll_allCells[irr-step,icc-step,PREC]
                else:
                    ...
                    if (mdl.success == True) and (mdl.x[1]>0.1) and (mdl.x[1]<3.9) and (mdl.x[1]!=x0[1]):
                        omega[ir:ir+step, ic:ic+step, PREC] = mdl.x[1];#mdl[1];
                        print(mdl)
                    else:
                        omega[ir:ir+step, ic:ic+step, PREC] = conf.step1_omegaOptPerPoll_allCells[irr-step,icc-step,PREC]
                        print('taking first guess')
