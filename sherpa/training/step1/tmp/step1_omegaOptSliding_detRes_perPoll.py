...
def computeOutput(ir,ic,Ide,rf, nx, ny, Prec, PREC, Indic, ICvecalpha, ICvecomega, rad,poly,lat):
    ...
    bndstmp_omega = (x0tmp_omega - 0.5, x0tmp_omega + 5.5)
    # bnds = ((0, 2), bndstmp)
    bnds = (bndstmp_alpha, bndstmp_omega)

    # a=time.time()
    opts = {'disp': False, 'ftol': 10e-6}
    mdl = minimize(iop2, x0, args=(inp1, inp2, rad, F), bounds=bnds, method='Nelder-Mead',
...
def iop2(beta, inp1, inp2, rad, F):
    ...
    # print(x.shape)
    # print(y.T.shape)
    # print(np.sqrt(np.mean(((x - y.T) ** 2))))
    return np.sqrt(np.mean(((x - y.T) ** 2)))
...
#     lat = f7.from7to28(conf.y);
    ...
    omega = np.full([conf.ny,conf.nx,conf.nPrec],np.nan);
    alpha = np.full([conf.ny,conf.nx,conf.nPrec],np.nan);
    ci2 = np.empty((conf.nPrec), dtype=object);
    CovB2 = np.empty((conf.nPrec), dtype=object);
